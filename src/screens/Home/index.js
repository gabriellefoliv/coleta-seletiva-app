...
const Home = ({ navigation }) => {
  ...
  const handleAccessQuiz = async () => {
    ...
    try {
      ...
      const response = await api.get(`/quizAccess`, {
        params: { codCliente: user.codCliente, dataResposta: hoje },
      });
      ...
      Alert.alert(
        "Erro",
        "Ocorreu um erro ao acessar o quiz. Tente novamente."
      );
  ...
  const fetchColeta = async () => {
    try {
      ...
      console.log(response.data[0].totalPeso);
      if (response.data[0].totalPeso === null) {
        setColeta(0);
      } else {
        setColeta(response.data[0].totalPeso);
      }
    } catch (error) {
      console.error("Erro ao carregar o peso coletado", error);
      if (error.status === 404) {
        setColeta(0);
      }
    }
    ...
    if (user && user.codCliente) {
      try {
        ...
        const cliente = response.data.find(
          (cliente) => cliente.codCliente === user.codCliente
        );
    ...
    <>
      ...
      <View style={styles.container}>
        <View style={styles.topContainer}>
          <View>
            ...
            <Text style={styles.topText}>
              Posição no Ranking :
              {!isNaN(position) ? ` ${position}º` : ` ${position}`}
            </Text>
          </View>
        </View>
        <View style={styles.bottomContainer}>
          ...
          <TouchableOpacity
            style={styles.quizButton}
            onPress={() => navigation.navigate("HistoricoColetas")}
          >
